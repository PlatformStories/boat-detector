...
| min_size | String | Minimum boat candidate size in m2. Default is 1000. | False |
| max_size | String | Maximum boat candidate size in m2. Default is 10000. | False |
...
+ Try to provide an accurate water mask as input. The built-in algorithm to derive the water mask is very simple. It relies on the normalized difference water index between the first band and the furthest NIR band. This can work well in some cases, e.g., calm blue water, but could fail miserably in choppy or green water, or when shadows from buildings are cast onto the water (in which case the water mask leaks onto the land).
